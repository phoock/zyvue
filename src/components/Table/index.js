...
import { tableList } from "./index.less";
/*eslint-disable*/
export default {
  name: "STable",
  data() {
  ...
  methods: {
      ...
      if (
        ...
        result.then((r) => {
          ...
          try {
            if (
              ["auto", true].includes(this.showPagination) &&
              r.totalCount <= r.pages * this.localPagination.pageSize
            ) {
    ...
    formatTableData(data) {
      ...
      const dataIndax = this.columns.map((item) => {
        return item.dataIndex;
      });
      dataIndax.forEach((item) => {
        dataObject.records &&
          dataObject.records.forEach((v) => {
            v[item] = !v[item] ? "-" : v[item];
          });
      });
      return dataObject;
    },
  ...
  render() {
    ...
    return (
      <div class="tableList" className={tableList}>
